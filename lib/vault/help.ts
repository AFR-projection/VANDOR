...
export const VAULT_MODE_COMMANDS: VaultHelpCommand[] = [
  ...
  { cmd: "list image · video · pdf · pinned", desc: "filter tipe / favorit", group: "navigasi" },
  ...
  { cmd: "move <id> folder:<nama>", desc: "pindah file ke folder", group: "organisasi" },
  { cmd: "read <nama|id>", desc: "baca & preview file", group: "file" },
  { cmd: "add", desc: "upload file baru", group: "file" },
  { cmd: "#T <isi catatan>", desc: "simpan catatan teks langsung", group: "file" },
  ...
  { cmd: "bulk tag <tag> <id...>", desc: "tambah tag ke banyak file", group: "bulk" },
  { cmd: "bulk delete tag:<tag>", desc: "hapus massal by tag", group: "bulk" },
  { cmd: "bulk delete type:image", desc: "hapus massal by tipe", group: "bulk" },
...
export function formatVaultHelpText(): string {
  const lines: string[] = ["**Perintah Vault Mode** — ketik `i` kapan saja untuk buka daftar ini."];
  ...
  lines.push("", "_Chat Mode:_ `/v` masuk · `/share-to-ai <id>` bagikan file ke AI.");
  return lines.join("\n");
}
