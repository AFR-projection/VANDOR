...
export function makeAssistantTools(userId: string, chatId: string) {
  ...
  const searchDbTool = tool({
    description:
      "Semantic search across user memories and tasks. Use when user asks 'pernah bahas?', 'ingat gak?', or needs past context. NOTE: Vault files are isolated and never returned by this tool.",
    inputSchema: z.object({
