...
import { Loader2Icon, ShieldIcon, SparklesIcon } from "lucide-react";
...
function tabFromHash(): GeneralTabId | null {
  ...
  return validTabs.includes(raw as GeneralTabId) ? (raw as GeneralTabId) : null;
}
      ...
      {tab === "security" && (
        <>
          <section className="space-y-3 rounded-xl border border-border/40 bg-card/30 p-4">
            ...
            <p className="text-xs text-muted-foreground">
              Sesi login: {Math.round(gate.ttlSeconds / 86_400)} hari · Sumber:{" "}
              <SourceBadge source={secrets.pin.source} />
              {secrets.pin.source === "database" && (
                <>
                  {" "}
                  · PIN dari .env tidak dipakai lagi setelah disimpan di sini
                </>
              )}
            </p>
